...
class Vecteur2D {
private:
    Point2D origine;  // Origine du vecteur
    Point2D direction; // Extrémité du vecteur

public:
    // Constructeurs
    Vecteur2D(const Point2D& p1 = Point2D(), const Point2D& p2 = Point2D()) 
        : origine(p1), direction(p2) {}

    // Accesseurs
    Point2D getOrigine() const { return origine; }
    Point2D getDirection() const { return direction; }

    void setOrigine(const Point2D& p) { origine = p; }
    void setDirection(const Point2D& p) { direction = p; }
    ...
    Vecteur2D rotation(double angle) const {
